#define SCORE(i, j) input_itemsets_l[j + i * (BLOCK_SIZE_RODINIA+1)]
#define REF(i, j)   reference_l[j + i * BLOCK_SIZE_RODINIA]
...
{  
	...
	//int bx = get_global_id(0)/BLOCK_SIZE_RODINIA;
	...
	int index   =   base + cols * BLOCK_SIZE_RODINIA * b_index_y + BLOCK_SIZE_RODINIA * b_index_x + tx + ( cols + 1 );
	int index_n   = base + cols * BLOCK_SIZE_RODINIA * b_index_y + BLOCK_SIZE_RODINIA * b_index_x + tx + ( 1 );
	int index_w   = base + cols * BLOCK_SIZE_RODINIA * b_index_y + BLOCK_SIZE_RODINIA * b_index_x + ( cols );
	int index_nw =  base + cols * BLOCK_SIZE_RODINIA * b_index_y + BLOCK_SIZE_RODINIA * b_index_x;
	...
	for ( int ty = 0 ; ty < BLOCK_SIZE_RODINIA ; ty++)
	...
	for( int m = 0 ; m < BLOCK_SIZE_RODINIA ; m++){
	...
	for( int m = BLOCK_SIZE_RODINIA - 2 ; m >=0 ; m--){
   
	  if ( tx <= m){
 
		  int t_index_x =  tx + BLOCK_SIZE_RODINIA - m ;
		  int t_index_y =  BLOCK_SIZE_RODINIA - tx;
